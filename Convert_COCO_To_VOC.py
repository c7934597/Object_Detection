...
target_class = ["mouse", "keyboard", "cell phone"]
...
# output
datasetPath = "../datasets/coco_2017/"
imgPath = "images/"
...
def generateXML(imgfile, fullpath, bboxes, imgfilename):
    xmlObject = ""
    for (labelName, bbox) in bboxes:
        ...
        with open(xml_file) as file:
            ...
            xmlfile = xmlfile.replace("{FILENAME}", imgfilename)
            xmlfile = xmlfile.replace("{PATH}", fullpath)
...
def makeLabelFile(filename, bboxes, imgfile):
    ...
    xmlContent = generateXML(imgfile, os.path.join(datasetPath, labelPath, xmlFilename), bboxes,
                             jpgFilename)
